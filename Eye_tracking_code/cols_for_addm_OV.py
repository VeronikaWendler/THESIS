...
version = 2 # 1 runs first code, 2 runs second code, I am too lazy for some other idea but 1 needs to be proeccessed first


if version == 1:
    # loading large Garcia csv with eyetracking data from both response and feedback phase
    data = pd.read_csv('D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/data/data_sets/OVParticipants_Eye_Response_Feed_Allfix_EXP_1_2_3_4.csv')  
...
#-------ALLFIX eyetracking data (all fixations in the trial; no restrictions)-----------------------------------------------    
    ...
    print(data[['p1', 'p2', 'V_corr', 'V_sub', 
                ...
                'DwellTimeAdvantage', 'DwellTimeAdvantage_Feed', 'DwellTimeAdvantage_allfix', 'FixationAdvantage', 'DwelltimeAdvantageCorrect'
                ]])

    data.to_csv('D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/data/data_sets/OVParticipants_Eye_Response_Feed_Allfix_addm_EXP_1_2_3_4_CCT.csv', index=False)
