...
def gather_fixseq(row, back=4):
    ...
    The function assumes `row.Fixations` is either a list of dicts or a string
    that can be safely parsed by ast.literal_eval, where each dict contains:
        - 'start_time' (in ms, relative to the first fixation)
        - 'roi' in {1:'E', 2:'S', ...}
    and `row.rtime` is the response time in seconds.
    """
    ...
    t0    = fixes[0]['start_time']
    rt_ms = row.rtime * 1000  # click (relative, ms)
    ...
    # reverse to: click (idx 0), then -1, -2, ...
    seq = seq[::-1]
    return seq[:back + 1]
...
# ------------------------------------------------------------------------
MAX_BACK = 4
raw_ES            = raw.copy()  # (comment: phase == 'ES' if `raw` already filtered upstream)
raw_ES['fix_seq'] = raw_ES.apply(lambda r: gather_fixseq(r, back=MAX_BACK), axis=1)

raw_ES = raw_ES[raw_ES.fix_seq.apply(len) > 1]
if raw_ES.empty:
    raise RuntimeError("No ES trial has ≥2 usable fixations after exclusions.")
...
# ------------------------------------------------------------------------
# 5) paired t-tests: is E_ratio ≠ S_ratio across subjects per fixation idx?
# ------------------------------------------------------------------------
ttest_results = []
...
# ------------------------------------------------------------------------
# 6)  export table (mean ± SEM + t/p values)
# ------------------------------------------------------------------------
...
print(f"Grand-mean fixation table with stats saved → {grand_csv}")