...
raw = (df_raw
         .query("phase == 'ES' and p2 != 50")      
         .copy())

# pie_split = 1  (p2  > 50 %)   |  0  (p2  < 50 %)
raw['pie_split'] = (raw['p2'] > 50).astype(int)
...
def trial_bins_EvS_byPie(row):
    ...
    out['trial']     = row.TrialID
    out['bin']       = out.index
    out['pie_grp']   = 'pie>0.5' if row.pie_split == 1 else 'pie<0.5'
    out['OV_lvl']    = row.OV_2
    out['VD_lvl']  = row.VD_2
    out['ratio_E']   = out.E_cnt / (out.E_cnt + out.S_cnt)
    out['ratio_S']   = 1 - out['ratio_E']
    return out[['sub','trial','bin','pie_grp','OV_lvl','VD_lvl','ratio_E','ratio_S']]
...
pie_tbl = raw.apply(trial_bins_EvS_byPie, axis=1).dropna()
pie_tbl = pd.concat(pie_tbl.tolist(), ignore_index=True)
pie_tbl['time_ms'] = pie_tbl['bin'] * BIN_MS
pie_tbl = pie_tbl[pie_tbl.time_ms <= rt_glob_mean + 100]

# subject-level
sub_pie = (pie_tbl
           .groupby(['sub','pie_grp','time_ms'])
           .agg(E_mean=('ratio_E','mean'),
                S_mean=('ratio_S','mean'))
           .reset_index())
...
colours = {'pie>0.5':'mediumorchid', 'pie<0.5':'darkturquoise'}

for grp, df in grp_pie.groupby('pie_grp'):
    c = colours[grp]
...
ax.set(xlim=(0, rt_glob_mean+100), ylim=(0,1),
       xlabel='Time (ms)', ylabel='Fixation ratio',
       title='ES phase – gaze E vs S, split by pie-probability')
ax.legend(frameon=False, fontsize=7, ncol=1,
          bbox_to_anchor=(1.02,1), loc='upper left')
fig.tight_layout()
fig.savefig(os.path.join(fig_dir, 'ES_EvS_byPie.png'),
            dpi=300, bbox_inches='tight')
plt.show()
...
sub_pie = (pie_tbl 
           .groupby(['sub','OV_lvl','pie_grp','time_ms'])
           .agg(E_mean=('ratio_E','mean'),
                S_mean=('ratio_S','mean'))
           .reset_index())
grp_pie_OV = (sub_pie
              .groupby(['OV_lvl','pie_grp','time_ms'])
              .agg(E_m=('E_mean','mean'),
                   E_sem=('E_mean', lambda x: x.std(ddof=1)/np.sqrt(len(x))),
                   S_m=('S_mean','mean'),
                   S_sem=('S_mean', lambda x: x.std(ddof=1)/np.sqrt(len(x))))
...
overlay_OV(grp_pie_OV, 'pie_grp',
           'Gaze E vs S – split by pie probability (OV overlay)',
           'OVoverlay_PieSplit.png',
           solid_tag='pie>0.5', dashed_tag='pie<0.5',
           solid_lab='pie>0.5 | gaze E', dashed_lab='pie>0.5 | gaze S')
