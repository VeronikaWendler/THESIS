...
plt.title('Mean confidence for E-options by objective probability - EXP1 (Garcia)')
plt.tight_layout()
plot_path = os.path.join(out_dir, 'mean_conf_violin.png')
plt.savefig(plot_path, dpi=300)
plt.close()

# ANOVA text summary
anova_txt = os.path.join(out_dir, 'anova_results.txt')
with open(anova_txt, 'w') as f:
    f.write(aov.summary().as_text())
print(f"Saved ANOVA summary to {anova_txt}")

# post-hoc to CSV
posthoc_csv = os.path.join(out_dir, 'posthoc_results.csv')
...
excel_path = os.path.join(out_dir, 'stats_results.xlsx')
