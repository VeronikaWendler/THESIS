...
eye_metrics <- c("DwellTimeAdvantage")  #DwellTimeAdvantage
...
p <- ggplot(summary_df,
  ...
  geom_errorbar(
    ...
    width = 0.2,
    linewidth = 0.8,
    color = "darkorchid"   # <- this makes the error bars darkorchid
  ) +
  geom_hline(
    yintercept = 0.5,
    linetype = "dotted",
...
ggsave(
  "D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/R_regression_eye_metrics_OV_Abs/ES_DTA_no_OV.pdf",
  plot = p,
  width = 5,
  height = 6,
  device = "pdf",
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/R_regression_eye_metrics_OV_Abs/ES_DwelltimeAdvantage_Results_no_OV.txt"
...
data <- read.csv(file) %>%
  ...
  drop_na(DwellTimeAdvantage, OVcate_2, chose_right) %>%
  mutate(chooseE = as.integer(chose_right == 0))

# ----------------------------
# 2. CREATE RT QUANTILE BINS
# ----------------------------
rt_breaks <- quantile(data$rtime, probs = seq(0, 1, length.out = 6), na.rm = TRUE)
...
# ----------------------------
# 3. DESCRIPTIVE BIN PROBABILITIES
# ----------------------------
agg <- data %>%
  group_by(sub_id, rtime_bin) %>%
  summarise(
...
# ----------------------------
# 4. FIT GLMM ON SAME DATA
# ----------------------------
data <- data %>%
  mutate(log_rtime = scale(log(rtime)))

m <- glmer(chooseE ~ log_rtime + (1 | sub_id),
           data = data, family = binomial)

# ----------------------------
# 5. GET MODEL PREDICTIONS AT BIN MIDPOINTS
# ----------------------------
rt_mid <- (head(rt_breaks, -1) + tail(rt_breaks, -1)) / 2
log_mid <- scale(log(rt_mid), center = attr(data$log_rtime,"scaled:center"),
                               scale = attr(data$log_rtime,"scaled:scale"))

preds <- emmeans(m, ~ log_rtime,
                 at = list(log_rtime = log_mid),
                 type = "response") %>% 
  as.data.frame()

# FIX: assign bin labels correctly
preds$rtime_bin <- factor(
  1:5,
  labels = c("0-20%", "20-40%", "40-60%", "60-80%", "80-100%")
)

# preds now has: prob, lower.CL, upper.CL, SE, and matches bins exactly
...
# ----------------------------
# 7. PRINT MODEL-BASED PROBABILITIES
# ----------------------------
print(preds)
...
overall_fix_stats <- fix_long %>%
  summarise(
    ...
    sd_left   = sd(y),     # SD of that proportion
    n      = n()
  )
...
dat <- read.csv("D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/FixSimulation_Study1/ppc_full_bayesian/sim_pure_aDDM_postdraws2/allSubjectLevel.csv")
