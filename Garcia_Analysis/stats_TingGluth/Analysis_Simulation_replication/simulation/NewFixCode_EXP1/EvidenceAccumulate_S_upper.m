...
% succesful function:
%         if FixR(k_fix) == 1
%             dv(k_fix) = beta2.*(Vr-Vl) - beta3.*(Vr-Vl); 
%         else
%             dv(k_fix) = beta4.*(Vr-Vl) - beta2.*(Vr-Vl); 
%         end

% works 
%         if FixR(k_fix) == 1
%             dv(k_fix) = beta2.*(Vr-Vl) + beta3.*(Vr-Vl) + beta4.*(Vr-Vl); % for each ms. In HDDM, the smallest unit is s.
%         else
%             dv(k_fix) = beta2.*(Vr-Vl) + beta3.*(Vr-Vl) + beta4.*(Vr-Vl); % for each ms. In HDDM, the smallest unit is s.
%         end
% 
% 
        ...
        % this below worked well but not sure yet not toatlly well
%         if FixR(k_fix) == 1
%             dv(k_fix) = beta1 + beta2.*Vr - (beta4.*Vl - beta3.*Vl); % for each ms. In HDDM, the smallest unit is s.
%         else
%             dv(k_fix) = beta1 + (beta4.*Vr - beta3.*Vr) - beta2.*Vl; % for each ms. In HDDM, the smallest unit is s.
%         end
%---------------------------------------------------------------------------------------------------------
% Accumulation function 2
...
% accumulation function for  different versions of the ddm similar to
% Cavanagh's paper
% 
%         if FixR(k_fix) == 1
%             dv(k_fix) = beta2.*(Vl-Vr) + beta3.*(Vl-Vr);    
%             % v ~ 0 + Value_diff + DTA;
%         else
%             dv(k_fix) = beta2.*(Vl-Vr) + beta3.*(Vl-Vr);   
%             % v ~ 0 + Value_diff + DTA;  
%         end
%        if FixR(k_fix) == 1
%             dv(k_fix) = (beta2.*Vl - beta3.*Vr) + (beta4.*Vl- beta5.*Vr);    
%             % v ~ 0 + Value_diff + DTA;
%         else
%             dv(k_fix) = (beta2.*Vl - beta3.*Vr) + (beta4.*Vl- beta5.*Vr); 
%             % v ~ 0 + Value_diff + DTA;  
