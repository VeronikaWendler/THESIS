...
% the E option (for analysis purposes E is always the left option; so when
...
z     = params(3);
beta2 = params(4)./1000;       % intercept
beta3 = params(5)./1000;       % attention or for ddm Value_difference
...
%         if FixR(k_fix) == 1
%             dv(k_fix) = beta1 + (beta3*Vr - beta5*Vl) - (beta2*Vr - beta4*Vl); % for each ms. In HDDM, the smallest unit is s.
%         else
%             dv(k_fix) = beta1 + (beta2*Vl - beta5*Vr + beta2*Vl + beta3*Vl; % for each ms. In HDDM, the smallest unit is s.
%         end
% 'v ~ 1 + AttentionW_E + AttentionW_S + InattentionW_E + InattentionW_S
...
% 
% %%----------------------------------------------------------------------------------------------------------------
%     if FixR(k_fix) == 1        % attending RIGHT
%         dv(k_fix) = beta3*Vl - beta2*Vr;   % pushes DOWN (toward right/lower)
%     else                        % attending LEFT
%         dv(k_fix) = beta2*Vl - beta3*Vr;   % pushes UP (toward left/upper)
%     end
% %%----------------------------------------------------------------------------------------------------------------
% % multi addm (dual attention + inattention model
%       if FixR(k_fix) == 1
...
sumdvALL = cumsum(rv) + spbias;      % spbias +  with the z bias - Chih-Chung's version
