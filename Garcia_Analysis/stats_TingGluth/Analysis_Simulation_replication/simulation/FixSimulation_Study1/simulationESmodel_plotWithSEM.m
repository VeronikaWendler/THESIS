function simulationESmodel_plotWithSEMOLD(pFix1, theta, sp, ...
                                       m_qRT, s_qRT, ...
                                       m_pS, s_pS, ...
                                       m_RT, s_RT, ...
...
if ~any(isfinite(m_RT)) || ~any(isfinite(s_RT))
    title(''); axis off;  % keep empty
else
    ...
    xlim([0.5,5.5])
    yl = [min(m_RT-s_RT)*0.95, max(m_RT+s_RT)*1.05];
...
if ~any(isfinite(m_corrProb)) || ~any(isfinite(sem_corrProb))
    title(''); axis off;  % keep empty
else
    ...
    xlim([0.5,5.5])
    yl = [max(0,min(m_corrProb-sem_corrProb)*0.95), min(1,max(m_corrProb+sem_corrProb)*1.05)];
    if all(isfinite(yl)) && yl(2) > yl(1)
        ylim(yl)
    end
end
