...
% these col names are saved differently depending on CSV or SIM file
...
% SIM:  sub_id, (no phase), RT, Choice(0=E,1=S), DwellDiff (S-E same as R-L)
  ...
  % already aggregated summary CSV
  ...
  exclude = [6, 14, 20, 26, 2, 9, 18];       % for EXP2 ..6, 14, 20, 26, 2, 9, 18  for EXP1: 1,4,5,6,14,99
...
% 4. Plot P(correct) by dwell advantage for correct option 
m_corrProb   = NaN(1,5);
sem_corrProb = NaN(1,5);

if isReal
    % CSV path (assumes these exact columns exist)
...
writetable(summaryStats, 'For_paper_6_metrics_summary_sim10.csv');
