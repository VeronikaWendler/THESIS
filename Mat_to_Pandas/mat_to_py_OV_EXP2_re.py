...
# this program runs through the .mat files for EXP2 for the OV experiment, 
# participant 1-26 and creates panda frames and csv files of the variables stored in .mat 
# because I am really bad at MATLAB I use python 
...
from toolbox import clean_column, rearrange_dataframe, clean_categorical_column
...
for file_path in mat_files:    
    ...
    if participant_match:
        ...
        raise ValueError(f"ID not found in path: {file_path}")
    ...
    # chose_right col, 1 if clicksArray1EXP == 0, else 0
    ...
    # Rearrange data for consistent op1/op2 structure
    df = rearrange_dataframe(df)
    csv_file_trials = os.path.join(participant_folder, f"EXP2_OV_participant_{int(participant_id)}_re.csv")
