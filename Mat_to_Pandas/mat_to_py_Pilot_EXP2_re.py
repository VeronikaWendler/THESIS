...
#Conversion code .mat -> pandas data frame -> csv for the Experiential Symbolic Phase (ES) of Garcia's direct replication
...
import re
import numpy as np
from toolbox import rearrange_dataframe, clean_categorical_column, clean_numeric_column
...
mat_files = sorted(glob.glob(os.path.join(mat_files_dir, "participant*_dataEXP2_*.mat")))
...
for file_path in mat_files:
    ...
    # cleaning columns
    left_imagesEXP2_vals = clean_categorical_column(left_imagesEXP2_vals)
    left_probabilityEXP2_vals = clean_numeric_column(left_probabilityEXP2_vals)
    right_imagesEXP2_vals = clean_categorical_column(right_imagesEXP2_vals)
    right_probabilityEXP2_vals = clean_numeric_column(right_probabilityEXP2_vals)
    selected_probabilityEXP2_vals = clean_numeric_column(selected_probabilityEXP2_vals)
    selected_imageEXP2_vals = clean_categorical_column(selected_imageEXP2_vals)
    selected_sideEXP2_vals = clean_categorical_column(selected_sideEXP2_vals)
    trialRunsEXP2_vals = clean_numeric_column(trialRunsEXP2_vals)
    trialIconsEXP2_vals = clean_categorical_column(trialIconsEXP2_vals)
    trialPiechartProbsEXP2_vals = clean_numeric_column(trialPiechartProbsEXP2_vals)
    ...
    df["ev1"] = (2.0 * df["left_probabilityEXP2"] - 1.0)
    df["ev2"] = (2.0 * df["right_probabilityEXP2"] - 1.0)
    
    df["p1"] = df["left_probabilityEXP2"]
    df["p2"] = df["right_probabilityEXP2"]
    ...
    df["cfout"] = -1
    # phase col, 'LE' in each row
    df["phase"] = "ES"

    # sess col, 0 as we only have one session
    df["sess"] = 0
    
    # `op1` and `op2` based on image type
    ...
    # rearranging the data so to get op1 (always E) and op2 (always S) structure
    df = rearrange_dataframe(df)
    csv_file_trials = os.path.join(participant_folder, f"EXP2_Pilot_participant_{int(participant_id)}_re.csv")
