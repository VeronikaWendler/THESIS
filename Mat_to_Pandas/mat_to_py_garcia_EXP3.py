# Veronika Wendler
# 11.12.24
#Conversion code .mat -> pandas data frame -> csv for the EE phase (EXP3) of Garcia's direct replication
...
import numpy as np
import toolbox
from toolbox import clean_column
...
# Get a list of all EXP1 .mat files for participants 1 to 26
...
# cleaning columns
    ...
    df["cfout"] = -1
    #phase col, "EE" for each row
    df["phase"] = "EE"
    # sess column, 1 for each row
    df["sess"] = 0
    df['op1'] = 'E'
    df['op2'] = 'E'
    ...
    csv_file_trials = os.path.join(participant_folder, f"EXP3_garcia_participant_{int(participant_id)}.csv")
