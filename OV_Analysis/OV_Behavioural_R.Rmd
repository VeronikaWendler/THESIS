...
# Behavioural Data Analysis - Experiment 2 (OV quasi replication)
```{r}
# libraries
library(tidyverse)
library(rstatix)
...
excluded_subs <- c(6, 14)   #6 and 14 have insuffiecient data... exclusion of participants from behavioral data
...
# Accuracy per block in LE
```{r}
# get the accuracy for each difficulty level in the LE phase (10/30, 20/60, 40/80, 70/90)

fig_out <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_Acc_LE_cond.pdf"    
...
# Accuracy per Block in LE Stats
```{r}

# cond is a factor
learning_acc_df <- learning_acc_df %>%
  mutate(cond = factor(cond, levels = c("30/10","60/20","80/40","90/70")))
...
write.csv(
  posthoc_tbl,
  "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/OV_table_LE.csv",
  row.names = FALSE
)
...
write.csv(
  desc_tbl,
  "D:/Aberdeen_Uni_June24/cap/THESIS/Pilot_Analysis/OV_table_LE_descriptives.csv",
  row.names = FALSE
)
...
fig_out_rt <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_RT_LE_cond.pdf"
ggsave(filename = fig_out_rt, plot = learning_rt_plot,
       width = 8, height = 6, dpi = 300)
learning_rt_plot
```

# Mean RT per block in LE stats
```{r}
learning_rt_df <- data %>% 
  filter(phase == "LE",
         cond %in% learning_cond,
...
# Accuracy (LE, ES) + repeated measures ANOVA
```{r}
anova_results <- accuracy_df %>%
  anova_test(dv = Mean_Accuracy, wid = sub_id, within = phase)
anova_table <- get_anova_table(anova_results)

fig_out <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_Acc_LE_ES_EE.pdf"     

# phase 
accuracy_df <- accuracy_df %>%
  mutate(phase = factor(phase, levels = c("LE", "ES", "EE")))

phase_colors <- c( "EE" = "deepskyblue"  ,"ES" = "darkorchid", "LE" = "rosybrown")
...
#accuracy_df %>%
#  cohens_d(Mean_Accuracy ~ phase, paired = TRUE) 
...
cohen.d(wide_df$LE, wide_df$ES, paired = TRUE) 
```
...
```{r}
#  mean rt per participant and phase
rt_df <- data_filt %>%
  group_by(sub_id, phase) %>%
...
fig_out <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_RT_LE_ES_EE.pdf"
...
fig_out <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_rt_E_S_options.pdf"
...
rtime_summary <- rtime_es %>%
  group_by(choice_type) %>%
  summarise(
    Mean = round(mean(Mean_RT), 3),
    SD = round(sd(Mean_RT), 3),
    .groups = "drop"
  )
...
fig_out <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/OV_EXP_Mean_Acc_E_S_options.pdf"
...
```
# glmer for choice accuracy E vs S
...
slope_data <- read.csv("D:/Aberdeen_Uni_June24/cap/THESIS/Basile_Garcia_Code/RetrieveAndCompareAnalysis-cleaning_VW/RetrieveAndCompareAnalysis-cleaning/data/stats/Fig2D_OV_SP.csv")
...
```{r}
#slope data from B. Garcia's code
slope_data <- read.csv("D:/Aberdeen_Uni_June24/cap/THESIS/Basile_Garcia_Code/RetrieveAndCompareAnalysis-cleaning_VW/RetrieveAndCompareAnalysis-cleaning/data/stats/Fig2D_OV_fullSP.csv")
...
```{r}
#slope data from B. Garcia's code
data <- read.csv("D:/Aberdeen_Uni_June24/cap/THESIS/Basile_Garcia_Code/RetrieveAndCompareAnalysis-cleaning_VW/RetrieveAndCompareAnalysis-cleaning/data/stats/Fig2C_OV.csv") %>%
  mutate(
    subject = as.factor(subject),
...
one_sample_tests <- data %>%
  ...
  summarise(
    mean_estimate = mean(p_est),
    sem           = sd(p_est) / sqrt(n()),
    t_test = list(t.test(p_est, mu = as.numeric(as.character(unique(p_obj))))),
    d_res  = list(effectsize::cohens_d(p_est, mu = unique(p_obj))),
    .groups = "drop"
  ) %>%
  mutate(
    ...
    cohen_d     = map_dbl(d_res,  "Cohens_d", .default = NA_real_),
    magnitude   = map_chr(d_res, "magnitude", .default = NA_character_)
  )%>%
  select(modality, p_obj, mean_estimate, sem, ci_low, ci_high,
