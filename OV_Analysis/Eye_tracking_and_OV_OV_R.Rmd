...
```{r}
file <- "C:/Cluster_Github/HDDM_Vero/data_sets/data_sets_OV/OVParticipants_Eye_Response_Feed_Allfix_addm_OV_Abs_CCT.csv"
data <- read.csv(file)
```
...
ggsave(
  filename = "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_Accuracy_Model_Plot.svg",
  plot     = plot_corr,
  device   = "svg",
  width    = 8,
  height   = 6,
...
output_txt <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_Accuracy_Model_Plot.txt"
sink(output_txt)
...
ggsave(
  filename = "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_Accuracy_Model_Plot.svg",
  plot     = plot_corr,
  device   = "svg",
  width    = 8,
  height   = 6,
...
# 2) Save the model summary to TXT (same path, but .txt)
output_txt <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_Accuracy_Model_Plot.txt"
sink(output_txt)
cat("### Linear Mixed Model for Accuracy (ES Phase) ###\n\n")
print(summary(model_corr))
sink()
...
ggsave(
  filename = "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_RTime_Model_Plot.svg",
  plot     = plot_rtime,
  device   = "svg",
  width    = 8,
  height   = 6,
...
txt_path <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_RTime_Model_Plot.txt"
sink(txt_path)
...
# 9) Save the model summary to TXT
txt_path <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_RTime_Model_Plot.txt"
sink(txt_path)
cat("### Linear Mixed Model for Reaction Times (ES Phase) ###\n\n")
print(summary(model_rtime))
sink()
...
# phase ES
phase_filter <- "ES"
excluded_subjects <- c(6, 14, 20, 26, 2, 9, 18)

output_dir <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/ES_eye_predict/"
...
quad_model_ES <- glmer(
  chose_right ~ 
    OV * (DwellTimeAdvantage + I(DwellTimeAdvantage^2)) +
    (1 | sub_id),
  data = data,
  family = binomial(link = "logit")
)

summary(quad_model_ES)
tab_model(quad_model_ES, transform = NULL)
```

```{r}
data$DTA_bin <- ntile(data$DwellTimeAdvantage, 5)

model_bins <- glmer(chose_right ~ DTA_bin + OV + (1|sub_id),
...
library(mgcv)
gam_model <- gam(
  chose_right ~ s(DwellTimeAdvantage) + OV,
  data = data,
  family = binomial
)
...
glm_base <- glm(chose_right ~ DwellTimeAdvantage + OV, 
                data=data, family=binomial)

seg_model <- segmented(glm_base, seg.Z = ~ DwellTimeAdvantage)
...
model_spline <- glmer(
  chose_right ~ ns(DwellTimeAdvantage, df = 3) + OV + (1 | sub_id),
  data = data,
  family = binomial
)
...
output_dir <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/EE_eye_predict/"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
...
ggsave("D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_DTA.pdf", 
...
agg_data <- agg_data %>%
  mutate(
    DTA_num = as.numeric(DwellTimeAdvantage_bin)  # 1 = E>>S ... 5 = S>>E
  )
...
# results folder
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_DwelltimeAdvantage_Results.txt"
sink(results_file) 
...
# ES -Dwell-Time Advantage (without OV)
...
ggsave(
  "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_DTA_no_OV.pdf",
  plot = p,
  width = 5,
  height = 6,
  device = "pdf",
...
# Polynomial trend across bins (collapsed across OV)
trend_model <- lmer(
  mean_p_choose_right ~ poly(DTA_num, 2, raw = FALSE) + (1 | sub_id),
  data = agg_data
)
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_DwelltimeAdvantage_Results_no_OV.txt"

sink(results_file)

cat("\n===== Polynomial Trend Mixed Model (DTA_num) =====\n")
print(summary(trend_model))

cat("\n===== ezANOVA: Effect of DwellTimeAdvantage_bin on P(Choose Right) =====\n")
print(anova_res)
...
# phase ES
phase_filter <- "ES"
excluded_subjects <- c(6, 14)   # exlcusion of participants that have bad eye-tracking data: 
...
# phase ES
phase_filter <- "ES"
excluded_subjects <- c(6, 14)    #6, 14, 20, 26, 2, 9, 18  - participants with behav. data issues: 6,14
...
## Subject-level probabilities for choosing E vs S
...
p <- ggplot(
  ...
  aes(x = rtime_bin,
      y = mean_prob,
      fill = Choice)   # use fill instead of color for bars
) +
...
ggsave(
  "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/Figures/ES_rtimeAdvantage_E_vs_S_bars.pdf",
  plot   = p,
  width  = 6,
  height = 5,
  device = "pdf",
...
excluded_subjects <- c(6, 14)  # this here is variable (one could also exclude the participants that have missing data in the eye-tracking code - theese would be more)  6, 14, 20, 26, 2, 9, 18
...
# Start from your `agg` object: sub_id × rtime_bin × Choice (E/S) × prob
# Reshape to wide format: one row per subject per bin
wide <- agg %>%
  pivot_wider(names_from = Choice, values_from = prob) %>%
  filter(!is.na(E) & !is.na(S))  # safety check

# Perform paired t-test per RT bin
bin_tests <- wide %>%
  group_by(rtime_bin) %>%
  summarise(
    t_test = list(t.test(S, E, paired = TRUE)),
    .groups = "drop"
  ) %>%
  mutate(
    ...
    mean_E = map_dbl(t_test, ~ mean(.x$data[[2]])),
    mean_S = map_dbl(t_test, ~ mean(.x$data[[1]])),
    diff   = mean_S - mean_E,
    ci_low = map_dbl(t_test, ~ .x$conf.int[1]),
    ci_up  = map_dbl(t_test, ~ .x$conf.int[2]),
    sig    = case_when(
      p < 0.001 ~ "***",
      p < 0.01  ~ "**",
      p < 0.05  ~ "*",
  ...
  select(rtime_bin, mean_E, mean_S, diff, t, df, p, ci_low, ci_up, sig)
...
# Fixation-by-fixation plot analysis
...
file_path <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/data/data_sets/OVParticipants_Eye_Response_Feed_Allfix_addm_OV_Abs_CCT.csv"
excluded_subjects <- c(6, 14, 20, 26, 2, 9, 18)
...
excluded_subjects <- c(6, 14, 20, 26, 2, 9, 18 )  #1, 4, 5, 6, 14, 99
...
ggsave("D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_DTA.pdf", 
       plot = p, width = 5, height = 6, device = "pdf", bg = "white"
       )
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_DwelltimeAdvantage_Results.txt"
sink(results_file) 
...
agg_data_complete <- agg_data %>%
  group_by(sub_id) %>%
  filter(n_distinct(DwellTimeAdvantage_bin) == length(unique(DwellTimeAdvantage_bin))) %>%
  ungroup()
...
ggsave(paste0("D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/", phase_filter, "_FinalFix.pdf"), 
       plot = p, width = 5, height = 6, device = "pdf", bg = "white")
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/ES_FinalFixations_Results.txt"
sink(results_file) 
...
file_path <- "C:/Cluster_Github/HDDM_Vero/data_sets/data_sets_OV/OVParticipants_Eye_Response_Feed_Allfix_addm_OV_Abs_CCT.csv"
data <- read.csv(file_path)
...
# value difference (Right - Left)
data$ValueDifference <- data$p2 - data$p1   # keep continuous (no rounding)

# Bin value difference into quantiles per subject
data <- data %>%
  group_by(sub_id) %>%
...
# -------- SUBJECT-LEVEL SE CALCULATION --------
# Step 1: subject-level means
per_sub <- data %>%
  group_by(sub_id, ValueDifference_bin, OVcate_2, FinalFixLocBinary) %>%
  summarise(p_choose_right = mean(chose_right, na.rm = TRUE), .groups = 'drop')

# Step 2: group-level mean and subject-level SE
summary_df <- per_sub %>%
  group_by(ValueDifference_bin, OVcate_2, FinalFixLocBinary) %>%
  summarise(
    mean_p = mean(p_choose_right, na.rm = TRUE),
    se     = sd(p_choose_right, na.rm = TRUE) / sqrt(n()),  # subject-level SE
    n_sub  = n(),
    .groups = 'drop'
  )
...
p <- ggplot(summary_df, aes(x = ValueDifference_bin, y = mean_p,
                            group = interaction(Fix_label, OV_label),
                            color = OV_label, linetype = Fix_label)) +
  geom_point(size = 3, position = position_dodge(width = 0.2)) +  
  geom_line(position = position_dodge(width = 0.2), linewidth = 1.2) +  
  geom_errorbar(aes(ymin = mean_p - se, ymax = mean_p + se),
                width = 0.2, linewidth = 0.8,
...
ggsave(paste0("D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/", 
              phase_filter, "_FinalFix.pdf"), 
       plot = p, width = 5, height = 6, device = "pdf", bg = "white")
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_FinalFixations_Results.txt"
sink(results_file) 
...
ggsave(paste0("D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/", phase_filter, "_NEW_FirstFixation_ValueDiff_Plot.svg"), 
       plot = p, width = 8, height = 6, device = "svg", bg = "white")
...
results_file <- "D:/Aberdeen_Uni_June24/cap/THESIS/OV_Analysis/stats_TingGluth/R_regression_OV_Abs/EE_FirstFixations_Results.txt"
sink(results_file) 
...
excluded_subs <- c(6, 14, 20, 26, 2, 9, 18)
...
```{r}
dat <- read.csv("D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/NewFixCode_EXp2/ppc_full_bayesian/sim_1000_postdraws2/allSubjectLevel.csv")
